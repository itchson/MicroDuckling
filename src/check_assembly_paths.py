...
cases=[('Battery','Chassis',V(-1,0,0),range(0,61,2)),
       ...
       ('ServoController','Chassis',V(1,0,0),range(0,61,2)),
       ('Buck_0','Chassis',None,[V(-i*.5,0,0) for i in range(5)]+[V(-2,0,i) for i in range(1,61)]),
       ('Buck_1','Chassis',V(0,-1,0),range(0,61,2)),
...
for name,frame,axis,steps in cases:
    ...
    for step in steps:
        ...
        q=ss[name].copy();q.translate(offset);v=q.common(ss[frame]).Volume
