...
The active mechanical source is `src/build_cad.py` with `body_r04.py`, `head_r04.py`, `electronics_r04.py` and `hardware_r02.py`. R04 module suffixes record their introduction; the current assembly is R05. Source dimensions are millimetres. Detailed vendor geometry is an optional local input; it is not part of the public CAD payload.
