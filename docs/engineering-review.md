...
The historical R05 IMU and servo-controller meshes adapted Adafruit PCB geometry under CC-BY-SA-3.0. The historical two regulator meshes were original dimension-based approximations under Apache-2.0, with photo-informed generic component arrangements; they do not use Pololu STEP geometry. [Component records](../components/records.json) preserve the original physical mass/COM estimates while describing the new display geometry; [attribution](../components/NOTICE.json) identifies the sources and changes. R06 removes the PCA9685 and second regulator; current component records contain the IMU and shared regulator only. The full intended assembly mass remains in metadata and must not be increased again when adding the visual records.
